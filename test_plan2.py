...
import select, termios, tty, time  #these are from teleop_keyboard script
#termios is for comms

#the following is from simple_gui
try:
...
#this is only for the conversion from c_value list element to target pose variables
...
        target_pose.pose.position.x = 0.150
        target_pose.pose.position.y = -0.132
...
        # -- dont use yet, this will close program
...
        #---- This is from MoveItPlanning Demo, __init__(self)
...
        '''# joint button
        tk.Button(self.frmLT, text="Go To Pos (Joint)", width=17, command=self.get_joint_input).grid(
            row=6, column=1, sticky="w", padx=3, pady=2
        )'''

        # coordination button
        tk.Button(self.frmRT, text="Go To Pos ()", width=17, command=self.get_coord_input).grid(
            row=6, column=1, sticky="w", padx=3, pady=2
        )

        '''# Gripper Buttons
        tk.Button(self.frmLB, text="Grip Open", command=self.gripper_open, width=8).grid(
            row=1, column=0, sticky="w", padx=3, pady=50
        )
        tk.Button(self.frmLB, text="Grip Close", command=self.gripper_close, width=8).grid(
...
    def set_layout(self): #layout setup
...
    def need_input(self):  #layout setup
        '''# 输入提示
...
        # 设置输入框的默认值
        # these are default joint angles and cartesian coords
        # from res angles and record coords arrays (all zeros at init)
        '''
...
        # joint 输入框
        #creates input fields for joint angles
        '''
...
        # coord 输入框
        #creates input fields for cartesian coords
...
        # 所有输入框，用于拿输入的数据
        #puts the data from the input fields into two arrays
...
        self.show_j1 = tk.Label(self.frmLC,textvariable=self.cont_1,font=("Arial", 9),width=7,
            height=1,bg="white",).grid(row=0, column=1, padx=0, pady=5)

        self.show_j2 = tk.Label(self.frmLC,textvariable=self.cont_2,font=("Arial", 9),width=7,
            height=1,bg="white",).grid(row=1, column=1, padx=0, pady=5)
        
        self.show_j3 = tk.Label(self.frmLC,textvariable=self.cont_3,font=("Arial", 9),width=7,
            height=1,bg="white",).grid(row=2, column=1, padx=0, pady=5)
        
        self.show_j4 = tk.Label(self.frmLC,textvariable=self.cont_4,font=("Arial", 9),width=7,
            height=1,bg="white",).grid(row=3, column=1, padx=0, pady=5)
        
        self.show_j5 = tk.Label(self.frmLC,textvariable=self.cont_5,font=("Arial", 9),width=7,
            height=1,bg="white",).grid(row=4, column=1, padx=0, pady=5)
        
        self.show_j6 = tk.Label(self.frmLC,textvariable=self.cont_6,font=("Arial", 9),width=7,
            height=1,bg="white",).grid(row=5, column=1, padx=5, pady=5)
...
        self.show_x = tk.Label(self.frmLC,textvariable=self.coord_x,font=("Arial", 9),width=7,
            height=1,bg="white",).grid(row=0, column=4, padx=5, pady=5)
        
        self.show_y = tk.Label(self.frmLC,textvariable=self.coord_y,font=("Arial", 9),width=7,
            height=1,bg="white",).grid(row=1, column=4, padx=5, pady=5)
        
        self.show_z = tk.Label(self.frmLC,textvariable=self.coord_z,font=("Arial", 9),width=7,
            height=1,bg="white",).grid(row=2, column=4, padx=5, pady=5)
        
        self.show_rx = tk.Label(self.frmLC,textvariable=self.coord_rx,font=("Arial", 9),width=7,
            height=1,bg="white",).grid(row=3, column=4, padx=5, pady=5)
        
        self.show_ry = tk.Label(self.frmLC,textvariable=self.coord_ry,font=("Arial", 9),width=7,
            height=1,bg="white",).grid(row=4, column=4, padx=5, pady=5)
        
        self.show_rz = tk.Label(self.frmLC,textvariable=self.coord_rz,font=("Arial", 9),width=7,
            height=1,bg="white",).grid(row=5, column=4, padx=5, pady=5)
...
        target_pose = PoseStamped()
        target_pose.header.frame_id = self.reference_frame
        target_pose.pose.position.x = itemgetter(0)(c_value)
        target_pose.pose.position.y = itemgetter(1)(c_value)
        target_pose.pose.position.z = itemgetter(2)(c_value)
        target_pose.pose.orientation.x = itemgetter(3)(c_value)
        target_pose.pose.orientation.y = itemgetter(4)(c_value)
        target_pose.pose.orientation.z = itemgetter(5)(c_value)
